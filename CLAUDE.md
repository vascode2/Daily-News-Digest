...
- AI summarization: Google Gemini API via `scripts/summarize-gemini.js`
...
npm run summarize
```
Reads the latest `tmp/raw-{key}.json`, `config/format.md`, and `agents/summarizer.md`, then writes `tmp/summaries-{key}.md` using Gemini (`gemini-3.5-flash` by default).
